...
UNIT_FIELD_ANGER = 119	-- 愤怒值
UNIT_FIELD_USE_RESPAWN_MAPID = 120	-- 使用复活丹的地图id
UNIT_FIELD_PROCESS_TIME = 121	--  进度条读完的时间戳
UNIT_FIELD_PROCESS_SECONDS = 122	--  读条总共需要时间	
UINT_FIELD_BOSS_DATA = 123	-- boss数据byte 0:boss状态 1:boss前一状态 2:boss索引(云游的是这个用途)
UINT_FIELD_BOSS_DATA1 = 124	-- boss数据byte 0:当前坐标x 1:当前坐标y 2:下一坐标x 3:下一坐标y(云游的是这个用途)
UINT_FIELD_VIEW_MODE = 124	-- 是否是观看模式
UINT_FIELD_VIRTUAL_CAMP = 125	-- 虚拟阵营
UINT_FIELD_XIANFU_INFO = 126	-- 跨服仙府的信息4bytes (0:宝箱数量, 1:死亡次数)
UINT_FIELD_VIP_LEVEL = 127	-- VIP等级
UINT_FIELD_VIP_TIME_OUT = 128	-- VIP超时时间
UINT_FIELD_VAMPIRIC = 129	-- 当前战斗的吸血值
UINT_INT_FIELD_ILLUSION = 131	-- 角色幻化
UINT_FIELD_BOOS_CUR_SPELLID = 132	-- 当前正在释放的蓄力技能idUNIT_INT_FIELD_APPEARANCE
UINT_FIELD_BOOS_CUR_SPELL_TIME = 133	-- 前一个技能结束的时间戳
UINT_FIELD_BOOS_CUR_SPELL_LAST_COUNT = 134	--  持续技能数量
UINT_INT_FIELD_PASSIVE_SPELL_START = 135	-- 被动技能开始
UINT_INT_FIELD_PASSIVE_SPELL_END = 235	-- 被动技能结束
UINT_INT_FIELD_WINGS_RANK = 235	-- 神羽阶数
UNIT_INT_FIELD_APPEARANCE = 236	--  外观
MAX_UNIT_FIELD = 237
...
TRIAL_INSTANCE_FIELD_BOSS_REFRESHED = 46	-- 是否已经刷怪
TRIAL_INSTANCE_FIELD_LAST_RANDOM_TIMESTAMP = 47	-- 最近随机刷怪时间戳
-- 家族场景
